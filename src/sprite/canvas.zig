//! Vendored and adapted from ghostty (src/font/sprite/canvas.zig), MIT licensed. Original copyright Mitchell Hashimoto and ghostty contributors.
...
pub const Canvas = struct {
    ...
    /// The rendered sprite as a tightly-packed A8 bitmap, trimmed of
    /// empty margins. Caller owns the returned data.
    pub const Bitmap = struct {
        data: []u8,
        width: u32,
        height: u32,
        /// Trimmed margins, needed to compute placement offsets.
        clip_left: u32,
        clip_top: u32,
    };

    /// Trim and export the canvas as a tightly packed alpha bitmap.
    pub fn toBitmap(self: *Canvas, alloc: Allocator) Allocator.Error!Bitmap {
        self.trim();
        ...
        const data = try alloc.alloc(u8, @as(usize, region_width) * region_height);
        errdefer alloc.free(data);

        const src = std.mem.sliceAsBytes(self.sfc.image_surface_alpha8.buf);
        for (0..region_height) |y| {
            const src_row = (y + self.clip_top) * sfc_width + self.clip_left;
            @memcpy(
                data[y * region_width ..][0..region_width],
                src[src_row..][0..region_width],
            );
        }
    ...
    pub fn getContext(self: *Canvas) z2d.Context {
        var ctx = z2d.Context.init(std.Io.failing, self.alloc, &self.sfc);
